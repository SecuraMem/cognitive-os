          ...
          <div className="flex-1 p-10 flex flex-col justify-between relative bg-gradient-to-b from-transparent to-[#16161a]/30">
            ...
            <div className="flex justify-end items-center pt-8 border-t border-[#27272a]">
              {currentStep < 3 ? (
                ...
                <button
                  onClick={() => handleProvisionStep(3).then(() => setIsProvisioned(true))}
    ...
    <div className="flex h-screen bg-[#0a0a0c] text-white overflow-hidden transition-all duration-1000 animate-in fade-in">
      ...
      <aside className="w-64 glass border-r border-[#27272a] flex flex-col p-6 space-y-8">
        ...
        <nav className="flex-1 space-y-2 mt-8">
          <NavItem icon={<DashboardIcon />} label="Insights" active />
          <NavItem icon={<DatabaseIcon />} label="Brain Log" />
          <NavItem icon={<PeopleIcon />} label="Graph" />
      ...
      <main className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
        ...
        {/* Action Center - Simplified for brevity */}
...
function NavItem({ icon, label, active = false }: { icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <div className={`flex items - center space - x - 3 p - 3 rounded - xl transition - all cursor - pointer ${active ? 'bg-primary/10 text-primary border border-primary/20' : 'text-muted-foreground hover:bg-[#1f2937] hover:text-white'} `}>
      {icon}
